<?php

namespace Proxies\__CG__\App\Entity;

/**
 * DO NOT EDIT THIS FILE - IT WAS CREATED BY DOCTRINE'S PROXY GENERATOR
 */
class ListaCatalogoDetalle extends \App\Entity\ListaCatalogoDetalle implements \Doctrine\ORM\Proxy\InternalProxy
{
    use \Symfony\Component\VarExporter\LazyGhostTrait {
        initializeLazyObject as private;
        setLazyObjectAsInitialized as public __setInitialized;
        isLazyObjectInitialized as private;
        createLazyGhost as private;
        resetLazyObject as private;
    }

    public function __load(): void
    {
        $this->initializeLazyObject();
    }
    

    private const LAZY_OBJECT_PROPERTY_SCOPES = [
        "\0".parent::class."\0".'codigo_interno' => [parent::class, 'codigo_interno', null],
        "\0".parent::class."\0".'estado' => [parent::class, 'estado', null],
        "\0".parent::class."\0".'fecha_creacion' => [parent::class, 'fecha_creacion', null],
        "\0".parent::class."\0".'fecha_modificacion' => [parent::class, 'fecha_modificacion', null],
        "\0".parent::class."\0".'id' => [parent::class, 'id', null],
        "\0".parent::class."\0".'id_lista_catalogo' => [parent::class, 'id_lista_catalogo', null],
        "\0".parent::class."\0".'id_valor' => [parent::class, 'id_valor', null],
        "\0".parent::class."\0".'listaCatalogo' => [parent::class, 'listaCatalogo', null],
        'codigo_interno' => [parent::class, 'codigo_interno', null],
        'estado' => [parent::class, 'estado', null],
        'fecha_creacion' => [parent::class, 'fecha_creacion', null],
        'fecha_modificacion' => [parent::class, 'fecha_modificacion', null],
        'id' => [parent::class, 'id', null],
        'id_lista_catalogo' => [parent::class, 'id_lista_catalogo', null],
        'id_valor' => [parent::class, 'id_valor', null],
        'listaCatalogo' => [parent::class, 'listaCatalogo', null],
    ];

    public function __isInitialized(): bool
    {
        return isset($this->lazyObjectState) && $this->isLazyObjectInitialized();
    }

    public function __serialize(): array
    {
        $properties = (array) $this;
        unset($properties["\0" . self::class . "\0lazyObjectState"]);

        return $properties;
    }
}
